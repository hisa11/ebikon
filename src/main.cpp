...
void send_rs485_command() {
    ...
    ThisThread::sleep_for(1ms); // 1ms待機
    
    char command = 0x10; // Position read command
    rs485.write(&command, 1); // Send command
    
    ThisThread::sleep_for(1ms); // Wait briefly before switching back
